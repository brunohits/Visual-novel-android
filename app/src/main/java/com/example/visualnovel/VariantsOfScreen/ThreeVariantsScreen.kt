...
@Composable
fun ThreeVariantsScreen(screen: Screen, navController: NavController) {
    Box(
        modifier = Modifier
            .fillMaxWidth()
    ...
    Box(
        ...
        Button(
            onClick = { navController.navigate(Destination.someScreen.createRoute(screen.arrayOfVariants[2].nextId)) },
            colors = ButtonDefaults.buttonColors(backgroundColor = colorResource(id = R.color.lightBlue)),
            modifier = Modifier.fillMaxWidth()
        ) {
            Text(
                text = screen.arrayOfVariants[2].variantText,
                color = Color.White,
                fontSize = 25.sp,
                textAlign = TextAlign.Center,
                fontFamily = FontFamily(Font(R.font.roboto_medium))
